...
- (void)initialImport {
    ...
    for (NSURL *url in urls) {
        ...
        Patient *patient = [self.persistenceManager getPatientWithUniqueID:uniqueId];
...
- (NSURL *)urlForPatientRepresentation: (Patient *)p {
...
- (void)generatePatientFilesForICloud: (void (^ _Nullable)(bool success))callback {
    ...
    NSArray<Patient*> *patients = [self.persistenceManager getAllPatients];
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_BACKGROUND, 0), ^{
...
- (void)ubiquitousStoreDidUpdate:(NSNotification *)notification {
    ...
    for (NSMetadataItem *item in deletedItems) {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            Patient *p = [self.persistenceManager getPatientWithUniqueID:patientUniqueId];
