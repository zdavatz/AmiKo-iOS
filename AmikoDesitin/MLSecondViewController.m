...
- (NSString *) interactionsHtml
{
    ...
    if ([medBasket count]>1) {
        ...
        if ([sectionTitles count]<2) {
            if ([[MLConstants appLanguage] isEqualToString:@"de"])
                [interactionStr appendString:@"<p class=\"paragraph0\">Zur Zeit sind keine Interaktionen zwischen diesen Medikamenten in unseren Daten vorhanden.</p><div id=\"Delete_all\"><input type=\"button\" value=\"Interaktion melden\" onclick=\"deleteRow('Notify_interaction','InterTable',this)\" /></div><br>"];
            else if ([[MLConstants appLanguage] isEqualToString:@"fr"])
